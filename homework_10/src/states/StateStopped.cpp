...
IDroneState::ExecuteResult StateStopped::execute(
    ...
    float& t_acceleration)
{
    ...
    if (angle_small && speed_zero) {
        ...
        return IDroneState::ExecuteResult{ std::make_unique<StateTurning>(), ctx };
    }

    return IDroneState::ExecuteResult{ nullptr, ctx };
}

std::string StateStopped::name() const { return "Stopped"; }