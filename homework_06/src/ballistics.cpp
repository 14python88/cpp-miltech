...
-> BallisticsInput {
  ...
  if (input.zd <= 0) {
    std::cerr << "Altitude must be above zero!" << '\n';
    exit(EXIT_FAILURE);
  };
...
-> AmmoInput {
  ...
  strlcpy(ammo_name_cpy, ammo_name, 15);
  bool found = false;
  for (int n = 0; n < 5; ++n) {
  ...
  if (found == false) {
    std::cerr << "Unknown ammo type!" << '\n';
    exit(EXIT_FAILURE);
  };

  AmmoInput ammo_input{.mass = m, .drag = d, .lift = l};
...
-> Coord {
  ...
  if (!(acos_arg >= -1) || !(acos_arg <= 1)) {
    std::cerr << "acos argument out of range (-1,1)!" << '\n';
    exit(EXIT_FAILURE);
  };
  ...
  float h = attack_speed * t - (pow(t, 2) * drag * attack_speed) / (2 * mass) +
            ...
            pow(t, 4) *
              ((-6) * drag * drag * g * lift * mass * (1 + pow(lift, 2) + pow(lift, 4)) +
               3 * pow(drag, 3) * pow(lift, 2) * attack_speed * (1 + lift * lift) +
               6 * pow(drag, 3) * pow(lift, 4) * attack_speed * (1 + lift * lift)) /
              (36 * mass * mass * mass * pow((1 + pow(lift, 2)), 2)) +
            pow(t, 5) * (3 * pow(drag, 3) * g * mass * pow(lift, 3) - 3 * pow(drag, 4) * lift * lift * attack_speed * (1 + lift * lift)) /
              (36 * (1 + lift * lift) * pow(mass, 4));

  if (t <= 0 || h <= 0) {
    std::cerr << "Inadequate calculation value(s): t, h!" << '\n';
    exit(EXIT_FAILURE);
  };

  float D = sqrt(pow((target_x - xd), 2) + pow((target_y - yd), 2));
  if (D <= 0) {
    std::cerr << "Inadequate value D!" << '\n';
    exit(EXIT_FAILURE);
  };
  ...
  if ((h + acceleration_path) > D) {
    ...
    fire_coords = {.x = xd_mid + (target_x - xd_mid) * ratio_mid, .y = yd_mid + (target_y - yd_mid) * ratio_mid};
  }
  else {
    fire_coords = {.x = xd + (target_x - xd) * ratio, .y = yd + (target_y - yd) * ratio};
  };
  return fire_coords;
}
